...
export = (socket: Socket) => {
  ...
  socket.on('webContents-printToPDF', async (id, options = {}, path) => {
    ...
    fs.writeFile(path, buffer, (error: unknown) => {
      if (error) {
  ...
  socket.on('webContents-session-getAllExtensions', (id) => {
    ...
    electronSocket.emit(
      'webContents-session-getAllExtensions-completed' + id,
      chromeExtensionInfo
    );
  });
  ...
  socket.on(
    ...
    async (id, path, allowFileAccess = false) => {
      ...
      electronSocket.emit(
        'webContents-session-loadExtension-completed' + id,
        extension
      );
    }
  );
