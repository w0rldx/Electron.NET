...
export = (socket: Socket, app: Electron.App) => {
  ...
  socket.on('autoUpdater-autoDownload-get', () => {
    electronSocket.emit(
      'autoUpdater-autoDownload-get-reply',
  ...
  socket.on('autoUpdater-autoInstallOnAppQuit-get', () => {
    electronSocket.emit(
      'autoUpdater-autoInstallOnAppQuit-get-reply',
  ...
  socket.on('autoUpdater-allowPrerelease-get', () => {
    electronSocket.emit(
      'autoUpdater-allowPrerelease-get-reply',
  ...
  socket.on('autoUpdater-fullChangelog-get', () => {
    electronSocket.emit(
      'autoUpdater-fullChangelog-get-reply',
  ...
  socket.on('autoUpdater-allowDowngrade-get', () => {
    electronSocket.emit(
      'autoUpdater-allowDowngrade-get-reply',
  ...
  socket.on('autoUpdater-updateConfigPath-get', () => {
    electronSocket.emit(
      'autoUpdater-updateConfigPath-get-reply',
  ...
  socket.on('autoUpdater-currentVersion-get', () => {
    electronSocket.emit(
      'autoUpdater-currentVersion-get-reply',
  ...
  socket.on('autoUpdater-requestHeaders-get', () => {
    electronSocket.emit(
      'autoUpdater-requestHeaders-get-reply',
  ...
  socket.on('autoUpdaterCheckForUpdatesAndNotify', async (guid) => {
    autoUpdater
      ...
      .then((updateCheckResult) => {
        electronSocket.emit(
          'autoUpdaterCheckForUpdatesAndNotifyComplete' + guid,
          updateCheckResult
        );
      })
      .catch((error) => {
        electronSocket.emit(
          'autoUpdaterCheckForUpdatesAndNotifyError' + guid,
          error
        );
      });
  });
  ...
  socket.on('autoUpdaterCheckForUpdates', async (guid) => {
    autoUpdater
      ...
      .then((updateCheckResult) => {
        electronSocket.emit(
          'autoUpdaterCheckForUpdatesComplete' + guid,
          updateCheckResult
        );
      })
      .catch((error) => {
  ...
  socket.on('autoUpdaterDownloadUpdate', async (guid) => {
    ...
    electronSocket.emit(
      'autoUpdaterDownloadUpdateComplete' + guid,
      downloadedPath
    );
  });
